...
# Step 6: 计算语义相似度
# ------------------------
def assign_needs_by_semantic_similarity(result_df, label_embeddings, need_names, device):
    results = []
    for idx, row in tqdm(result_df.iterrows(), total=len(result_df), desc="分析中"):
        sem_embedding = embedding_model.encode(
            row["语义群"],
        ...
        # 转成 {label: score} 字典
        label_scores = {need_names[i]: cos_scores[i].item() for i in range(len(need_names))}
        ...
        results.append({
            ...
            "语义群": row["语义群"],
            "匹配需求": best_label,
            "相似度": label_scores[best_label],
            "label_scores": label_scores  # 新增完整字典
        })
...
def restructure_training_input(need_df: pd.DataFrame):
    ...
    for _, row in need_df.iterrows():
        ...
        for label, score in row["label_scores"].items():
            rows.append({
                "product": product,
...
# 新嵌入(结合label和句子)
def get_combined_label_embeddings(need_labels, embedding_model, alpha=0.5):
    label_embeddings = {}

    for label, sentences in need_labels.items():
        # 生成每条句子embedding
        sentence_vecs = embedding_model.encode(sentences, convert_to_tensor=True, normalize_embeddings=True)

        # mean pooling
        mean_vec = sentence_vecs.mean(dim=0)
        # max pooling
        max_vec = sentence_vecs.max(dim=0).values

        # 融合
        combined_vec = alpha * mean_vec + (1 - alpha) * max_vec

        label_embeddings[label] = combined_vec.cpu()  # 转CPU以节省显存并便于保存
